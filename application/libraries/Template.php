...
class Template {
    ...
    function display($template, $data = NULL, $js = NULL, $css = NULL) {
        ...
        if($status_link){
            ...
            $data['_fullcontent']   = $this->CI->load->view('template/ajax/fullcontent', $data);
        }else{
    ...
    function display_ajax($template, $data = NULL, $js = NULL, $css = NULL, $html=FALSE) {
        $data['_content']       = $this->CI->load->view($template, $data, TRUE);
        $data['_js']            = $this->CI->load->view('template/ajax/js', $js, TRUE);
        $data['_css']           = $this->CI->load->view('template/ajax/css', $css, TRUE);
        if($html){
            return $this->CI->load->view('template/ajax/content', $data, $html);
        }else{
            $this->CI->load->view('template/ajax/content', $data);
        }
    }
}
